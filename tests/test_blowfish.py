...
#   key bytes            cleartext           ciphertext
ref_values = (
...
class TestBlowfish:
    def test_cipher(self):
        for ref in ref_values:
            ...
            cleartext = ref[1]
            ciphertext = ref[2]
            cipher = blowfish.Cipher(key, 8)
            computed_ciphertext = cipher.encryption(cleartext)
            assert(computed_ciphertext == ciphertext)

            computed_cleartext = cipher.decryption(computed_ciphertext)
            assert(computed_cleartext == cleartext)
            del cipher
